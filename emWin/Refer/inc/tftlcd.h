...
typedef struct 
{
	...
	void (*ColorFill)(u16 sx,u16 sy,u16 ex,u16 ey,u16 *color);
	void (*ShowChar)(u16 x,u16 y,u8 num,u8 size,u8 mode);
	void (*ShowNum)(u16 x,u16 y,u32 num,u8 len,u8 size);
	void (*ShowxNum)(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode);//显示 数字
	void (*ShowString)(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p);//显示一个字符串,12/16字体
	
	void (*WriteReg)(u16 LCD_Reg, u16 LCD_RegValue);
	u16 (*ReadReg)(u16 LCD_Reg);
	void (*WriteRAMPrepare)(void);
	void (*WriteRAM)(u16 RGB_Code);
	void (*ScanDir)(u8 dir);//设置屏扫描方向
	void (*DisplayDir)(u8 dir);	//设置屏幕显示方向
	void (*SetWindow)(u16 sx,u16 sy,u16 width,u16 height);//设置窗口			
}LCDBase;

const extern LCDBase lcd;
